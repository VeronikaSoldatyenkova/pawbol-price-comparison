...
def _safe_preview_value(value):
    """Make unusual Excel values safe for Streamlit's frontend preview renderer."""
    if value is None:
        return None
    ...
    try:
        ...
        # Lists/dicts/arrays can make pd.isna return a non-scalar result.
        pass
    ...
    # Nested Python objects are uncommon in Excel but can appear after parsing.
    # Converting them prevents Arrow/DataFrame serialization from crashing the UI.
...
def _safe_preview_dataframe(df):
    """Return a small, frontend-safe preview instead of serializing the full sheet."""
    preview = df.iloc[:PREVIEW_ROWS, :PREVIEW_MAX_COLUMNS].copy()

    for col in preview.columns:
        if preview[col].dtype == object:
            preview[col] = preview[col].map(_safe_preview_value)

    return preview


def _render_preview_popover(df, label):
    """
    Render a preview without a checkbox/toggle.

    Opening a Streamlit popover does not rerun the whole application, which avoids
    the grey rerun overlay + occasional blank-screen issue seen with the old
    `Show first 10 rows` checkbox.
    """
    with st.popover(label, use_container_width=False):
        preview = _safe_preview_dataframe(df)
        st.caption(
            f"Showing first {min(PREVIEW_ROWS, len(df)):,} row(s)"
            + (
                f" and first {PREVIEW_MAX_COLUMNS} columns of {len(df.columns)}."
        ...
        st.dataframe(
            ...
            hide_index=True,
            height=min(390, 38 + max(1, len(preview)) * 35),
        )
...
def render_current_pricelist():
    ...
    with st.container(border=True):
        ...
        if our_type == "PowerBI Pricelist":
            ...
            with info_left:
                _render_preview_popover(
                    our_df,
                    "Preview first 10 rows — PowerBI pricelist",
                )
            with info_right:
            ...
            excluded = {
                col
                for col in [ean_column, sku_column, price_column]
                if col is not None
            }
            ...
            with preview_col:
                _render_preview_popover(
                    our_df,
                    "Preview first 10 rows — current pricelist",
                )
    ...
    for index, supplier_file in enumerate(supplier_files, start=1):
        ...
        with st.expander(
            ...
            with preview_col:
                _render_preview_popover(
                    supplier_df,
                    f"Preview first 10 rows — supplier {index}",
                )
